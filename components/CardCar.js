  ...
  return (
    <Link href={`/product-detail?user=${props.user}&ver=${props.ver}&title=${props.title}&desc=${props.desc}&model=${props.model}&price=${props.price}&year=${props.year}&km=${props.km}&city=${props.city}&img=${props.img}`}><div className='w-72 scale-100 overflow-hidden bg-transparent border-2 border-gray-200 rounded-sm p-4 pb-0 hover:cursor-pointer hover:scale-105 hover:shadow-xl duration-300'>
        ...
        <div className='border-t border-gray-200 flex items-center py-2 mt-4'>
          ...
          {props.city}</div><p className='text-[12px] font-extralight flex gap-1 items-center'>De: {props.user} <svg className={`text-blue-300 ${props.ver == "true" ? 'block' : 'hidden'}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="m8.6 22.5l-1.9-3.2l-3.6-.8l.35-3.7L1 12l2.45-2.8l-.35-3.7l3.6-.8l1.9-3.2L12 2.95l3.4-1.45l1.9 3.2l3.6.8l-.35 3.7L23 12l-2.45 2.8l.35 3.7l-3.6.8l-1.9 3.2l-3.4-1.45zm2.35-6.95L16.6 9.9l-1.4-1.45l-4.25 4.25l-2.15-2.1L7.4 12z"/></svg></p></p>
        </div>
    </div></Link>
  )
}
